...
import { Plus, Trash2, User, Calculator, Save, History, Download, TrendingUp, DollarSign, Package } from 'lucide-react';

type Item = { id: number; name: string; cost: string };
type Template = { id: string; name: string; items: Array<{ name: string; cost: string }> };
type Quote = {
  id: number | string;
  date: string;
  projectName: string;
  ...
  items: Array<{ id?: number; name: string; cost: string | number }>;
  customerType: string;
  margin: number;
  totalCost: number;
  quotedPrice: number;
...
export default function BusinessQuoteApp(): JSX.Element {
  const [activeTab, setActiveTab] = useState<string>('calculator');
  const [projectName, setProjectName] = useState<string>('');
  const [customerName, setCustomerName] = useState<string>('');
  const [customerPhone, setCustomerPhone] = useState<string>('');
  const [items, setItems] = useState<Item[]>([{ id: Date.now(), name: '', cost: '' }]);
  ...
  // Small storage helpers: prefer window.storage if available (some host apps provide it), fallback to localStorage
  const storageGet = async (key: string): Promise<string | null> => {
    try {
      // @ts-expect-error runtime unknown
      if (window.storage && typeof window.storage.get === 'function') {
        // some hosts return { value: '...' }
        // @ts-expect-error runtime unknown
        const res = await window.storage.get(key);
        if (!res) return null;
        if (typeof res === 'object' && typeof res.value !== 'undefined') return res.value;
        return res;
      }
    } catch (e) {
      // ignore and fallback
  ...
  const loadData = async () => {
    try {
      const templatesData = await storageGet('templates');
      const historyData = await storageGet('quote-history');
      ...
      if (templatesData) {
        ...
        await storageSet('templates', JSON.stringify(defaults));
      }
  ...
  const saveAsTemplate = async () => {
    ...
    const newTemplate: Template = {
      ...
      name: templateName,
      items: items.map((item: Item) => ({ name: item.name, cost: item.cost }))
    };

    const updated = [...savedTemplates, newTemplate];
    setSavedTemplates(updated);
    await storageSet('templates', JSON.stringify(updated));
    setShowSaveModal(false);
    setTemplateName('');
  };

  const deleteTemplate = async (id: string) => {
    const updated = savedTemplates.filter((t: Template) => t.id !== id);
    setSavedTemplates(updated);
    await storageSet('templates', JSON.stringify(updated));
  };
  ...
  const saveQuote = async () => {
    ...
    await storageSet('quote-history', JSON.stringify(updated));
  ...
  const deleteQuote = async (id: number | string) => {
    const updated = quoteHistory.filter((q: Quote) => q.id !== id);
    setQuoteHistory(updated);
    await storageSet('quote-history', JSON.stringify(updated));
  };
  ...
  const loadQuote = (quote: Quote) => {
    ...
    setCustomerType(quote.customerType || 'regular');
  ...
  const updateItem = (id: number, field: keyof Item | string, value: string) => {
    setItems(items.map((item: Item) =>
      item.id === id ? { ...item, [field]: value } as Item : item
    ));
  };
